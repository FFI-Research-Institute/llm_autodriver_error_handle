...
class PlanningMetric(Metric):
    ...
    def evaluate_single_coll(self, traj, segmentation, token=None):
        ...
        if collision.any() and len(collision) == 6 and token:
            # print(f'Collision at {token}')
            self.error_token.add(token)
            collision_time_steps = np.where(collision)[0]
            flag = True
            for ts in collision_time_steps:
                if 0 in segmentation[ts].cpu().numpy():
    ...
    def evaluate_coll(self, trajs, gt_trajs, segmentation, token=None):
        ...
        for i in range(B):
            gt_box_coll = self.evaluate_single_coll(gt_trajs[i], segmentation[i],
                                                    token=token)  # collision of trajectory points within the map
            ...
            m1 = torch.logical_and(m1, torch.logical_not(
                gt_box_coll))  # timestep where gt box points are not collision and within the map

            ti = torch.arange(n_future, device=trajs.device)
            obj_coll_sum[ti[m1]] += segmentation[i, ti[m1], yi[m1], xi[
                m1]].long()  # sum traj's collision points by timestep in batch, only sum collision points where gt box points are not collision

            m2 = torch.logical_not(gt_box_coll)  # not_collision of gt box points
            box_coll = self.evaluate_single_coll(trajs[i], segmentation[i], token=token)
            obj_box_coll_sum[ti[m2]] += (box_coll[ti[
                m2]]).long()  # sum box's collision points in timestep, only sum collision points where gt trajectory points are not collision
